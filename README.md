...
│   ├── synthesize_smoke_layer.py   Perlin-noise smoke / non-uniform fog layer
│   ├── run_marigold_depth.py       Marigold depth-map inference
...
└── third_party/                    pinned references for external deps (Marigold)
```

---

## 🛠️ Reproducing the haze synthesis

You only need this if you want to **rebuild the synthetic haze dataset** with different source images or rendering parameters — most users should just stream from Hugging Face.
...
with optional Perlin-noise modulation of optical thickness $\tau$ for non-uniform haze and valley fog ([`scripts/synthesize_smoke_layer.py`](scripts/synthesize_smoke_layer.py)).

Per-image depth maps that drive distance-aware scattering are generated by [`scripts/run_marigold_depth.py`](scripts/run_marigold_depth.py); see [`docs/depth_generation.md`](docs/depth_generation.md) for the Marigold setup and [`third_party/README.md`](third_party/README.md) for the pinned upstream commit.
